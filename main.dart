...
class App extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialApp(
        ...
        theme: ThemeData(colorScheme: ColorScheme.fromSeed(seedColor: const Color(0xFF6EB5FF)), useMaterial3: true),
        home: const Home(),
      );
}
...
class _AddPageState extends State<AddPage> {
  ...
  String computeRound(DateTime d) {
    ...
    if (day == 1) {
      final prev = DateTime(d.year, d.month - 1, 17);
      return DateFormat('yyyyMM').format(prev) + '_B';
    }
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(children: [
                Text(widget.two ? 'สรุปเลข 2 หลัก (00–99)' : 'สรุปเลข 3 หลัก (000–999)', style: const TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 2),
                Text('รอบ: ${''}', style: const TextStyle(fontSize: 12)),
              ])),
  ...
  Future<void> open() async {
    ...
    _db = await openDatabase(path, version: 1, onCreate: (db, v) async {
      await db.execute('''CREATE TABLE entries(
        ...
        category TEXT NOT NULL, -- TWO/THREE
